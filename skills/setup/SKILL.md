...
Read `${CLAUDE_PLUGIN_ROOT}/config.json` to see if there's an existing configuration. If a config already exists, show the user the current settings and ask if they want to reconfigure.
...
```bash
cat ${CLAUDE_PLUGIN_ROOT}/logs/notify.log
```
